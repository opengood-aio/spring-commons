        ...
        return try {
            beanRefresher.refresh(
                BeanRefreshConfig(
                    ...
                    classType = Class.forName(request.classType).kotlin,
                ),
            )
            ResponseEntity.ok(BeanRefreshResponse(message = "Successfully refreshed bean '${request.beanId}'"))
        } catch (e: Exception) {
            log.error("Unable to refresh bean '${request.beanId}'", e)
            ResponseEntity.badRequest().body(BeanRefreshResponse(message = "Unable to refresh bean '${request.beanId}'"))
        }
    }
